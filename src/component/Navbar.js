...
function Navbar(props) {
    return (
        <>
            <nav className={`navbar fixed-top shadow navbar-expand-lg navbar-${props.mode} bg-${props.mode}`}>
                <div className="container-fluid">
                    <a className="navbar-brand fw-bold" href="/">{props.title}</a>
                    <label className={`switch text-${props.mode === "dark" ? "light" : "dark"}`}>
                        ...
                        <input type="checkbox" onClick={props.toogleMode} className="input" />
                        <span className="slider bg-primary"></span>
                    </label>
                </div>
            </nav>
        </>
